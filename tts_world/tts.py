...
def main() -> None:
    ...
    flag = acoustic_predicted[:, 0] > 0.5
    lf0 = acoustic_predicted[:, 1]
    f0 = (np.exp(lf0) * flag).astype(np.float64)
    sp = np.exp(acoustic_predicted[:, 2 : 2 + 513]).astype(np.float64)
    ap = acoustic_predicted[:, 2 + 513 :].astype(np.float64)
